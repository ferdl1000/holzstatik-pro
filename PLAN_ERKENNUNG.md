...
- 2026-06-24 | Selbst-Lernen v2 | Regeln nach Projekt-NAME skopiert (transferierten nicht) | Planer-Key + serverseitig | Korrekturen werden nach PLANVERFASSER (Büro-Adresse als stabiler Key, z.B. "bahnhofstrasse-14c-8350-fehring") skopiert → transferieren auf nächsten Plan desselben Planers. Orchestrator lädt Regeln serverseitig (_shared/learning.ts), injiziert sie als "GELERNTE KORREKTUREN" in den KI-Prompt UND wendet sie deterministisch an. Verifiziert: load→inject→apply E2E. Planer-Key-Fallback aus Rohtext (derivePlanerKeyFromText) wenn KI keine Planer-Adresse liefert; globale Regeln (trigger_pattern=null) greifen immer.
- 2026-06-24 | Robustheit | dünne Extraktion still als "fertig" ausgegeben | Degradierte-Extraktion-Guard | Orchestrator misst echte Tiefe (Rohtext-Länge + parsedFacts-Signal, NICHT texts[].length) → Banner "möglicherweise unvollständig" statt falscher Sicherheit. Kein Fehlalarm bei thin-aber-vollständig.
- 2026-06-24 | Nöhrer | gescannter Plan, ° verloren | DN-Parser ohne Gradsymbol | Nöhrer = 5° Paneeldach ("6cm Dachpaneele, Dachneigung 5°"). Eine PDF-Version (ERPL_Nöhrer) ist reines Rasterbild (0 Text → nur KI-Vision). parseDachneigung liest jetzt auch "Dachneigung 5"/"DN 5" ohne ° (1-60°, nicht wenn cm/mm/m/% folgt).
- 2026-06-24 | Lebenbauer | Multi-Form-Plan | — | Lebenbauer hat reichen Text-Layer: FLACHDACH (14×) + SATTELDACH (4×) + VORDACH (9×) + Ziegel. Echter Multi-Roof-Testfall mit gemischten Dachformen.
- 2026-06-26 | Lechner | 6 Müll-Dachteile mit Textfragment-Labels ("ÜBERDACHUNG, STALL NEU, EINREICHPLAN.") + doppelte 9.5° | Label-Sanitizer + Dedup | Orchestrator: isGarbageLabel() wirft Textfragmente raus → saubere Namen (Hauptdach/Vordach N); Vordach-Ergänzung max 2; Dedup gleicher Dachteile (Form+Neigung+Maße); genau EIN Hauptdach. Ergebnis: 6 → 3 saubere Teile.
- 2026-06-26 | Kachel-Analyse | große Pläne / kleine Beschriftungen | renderPdfToTiles + multi-image | Client (pdfjs) zerlegt Plan in überlappende hochauflösende Kacheln (documents.tile_paths); agent-document liest ALLE in EINEM Gemini-Call (geminiVision images[]) mit HIGH-Auflösung. Fail-safe: ohne Kacheln Einzelbild wie bisher.
- 2026-06-26 | 3D | Dachteile überlappten (positionX/Y=0) | layoutRoofParts | Roof3D ordnet gleich-positionierte Teile als Reihe entlang Z an → gesamte Konstruktion sichtbar.
